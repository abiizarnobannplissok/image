            ...
            const getLabel = (idx: number) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative border transition-all flex flex-col items-center justify-center gap-2 overflow-hidden aspect-square
                    ${referenceImages[index] ? 'border-gray-600 cursor-grab active:cursor-grabbing shadow-lg' : 'border-dashed border-gray-700 hover:border-gray-400 bg-gray-900/40 cursor-pointer hover:bg-gray-800/60'}
                    ${dragActive === index ? 'border-white bg-white/10 scale-[1.02] shadow-xl' : ''}
                    ${isDeleteMode && referenceImages[index] ? 'ring-2 ring-red-500/50 cursor-pointer !border-red-500' : ''}
                    ${isPrimary && !referenceImages[index] ? 'border-blue-500/40 ring-1 ring-blue-500/20' : ''}
                    ${isSecondary && !referenceImages[index] ? 'border-purple-500/40 ring-1 ring-purple-500/20' : ''}
                  `}
                >
                <input 
                  type="file" 
                ...
                {referenceImages[index] ? (
                  <>
                    ...
                    />
                    {isDeleteMode && (
                      <div className="absolute inset-0 bg-red-900/40 backdrop-blur-[1px] flex items-center justify-center pointer-events-none">
                         <div className="bg-red-900 text-red-100 px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest border border-red-500 shadow-xl">
                           REMOVE
                         </div>
                      </div>
                    )}
                    <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-2 pointer-events-none transition-opacity duration-300 ${isDeleteMode ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'}`}>
                      <p className="text-[10px] font-bold text-white text-center uppercase tracking-tighter">{getLabel(index)}</p>
                    </div>
                  </>
                ) : (
                  <>
                    <svg className="w-8 h-8 text-gray-500 group-hover:text-gray-300 transition-colors flex-shrink-0 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path>
                    </svg>
                    <div className="flex flex-col items-center leading-tight">
                      <span className="text-[12px] font-black uppercase tracking-wider text-gray-400 group-hover:text-white transition-colors">
                        {getLabel(index)}
                      </span>
                      <span className="text-[10px] text-gray-600 group-hover:text-gray-400 transition-colors font-medium mt-0.5">
                        {isPrimary ? 'Style' : isSecondary ? 'Content' : 'Ref'}
                      </span>
                    </div>
                  </>
                )}
